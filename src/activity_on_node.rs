...
type Edge = Option<NonNull<Vec<NonNull<Node>>>>;
...
impl Node {
    fn new(cost: usize) -> Self {
        ...
        Self {
            next: None,
            prev: None,
            serial_number: unsafe { ID.to_string() },
    ...
    fn add_next(&mut self, next: &mut Node) {
    ...
    fn add_prev(&mut self, prev: &mut Node) {
...
impl Drop for Node {
    fn drop(&mut self) {
        ...
        println!("drop {}", self.serial_number);
...
mod tests {
    ...
    // cargo +nightly miri test -- activity_on_node

    #[test]
    fn test1() {
        let mut node1 = Node::new(2);
        let mut node2 = Node::new(2);
        node1.add_next(&mut node2);
    }
}
